...
function renderHostels(hostels) {
    ...
    hostels.forEach(data => {
        if (data.approval_hostel_status.trim().toLowerCase() === 'approved') {
...
function searchHostels() {
    ...
    var filteredHostels = myHostelApprovals.filter(function (hostel) {
        return hostel.name.toLowerCase().includes(searchInput) && hostel.approval_hostel_status.trim().toLowerCase() === 'approved';
    });
